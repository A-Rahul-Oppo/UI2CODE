...
def pair_visualization(pairs, img, img_shape, show_method='line'):
    ...
    if show_method == 'line':
        for id in pairs:
            pair = pairs[id]
            for p in pair:
                board = visualize(board, p.compos_dataframe, img_shape, attr='group_pair', show=False)
    elif show_method == 'block':
        for id in pairs:
            pair = pairs[id]
            for p in pair:
                board = visualize_block(board, p.compos_dataframe, img_shape, attr='group_pair', show=False)
